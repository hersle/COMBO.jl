...
which corresponds to modes that enters the horizon \emph{at} radiation-matter equality:
\begin{itemize}
\item
...
The increase for $l \lesssim 10^1$ is caused by the late-time ISW effect,
...
have reached the maximum of the first \emph{compression} at recombination.
...
the maximum of the first \emph{decompression} when the photons are released at recombination.
...
undergone multiple oscillations and reached the maximum of the $n$-th compression/decompression.
...
so the temperature anisotropies due to the SW effect decreases and the peaks become lower in amplitude.
\end{itemize}
...
During tight coupling \eqref{eq_perturb_tight}, it is in phase with the dipole $\Theta_1$.
Whereas the temperature power spectrum is largely sourced by overdensities (or $\Theta_0$),
this polarization power spectrum is largely sourced by fluid velocities (or $\Theta_1 \propto \Theta_2$),
so they are out of phase to a good approximation.
...
which becomes visible as a bump around $l \approx 100$.

\item
The amplitude of the EE signal is much lower,
...
in other words from modes whose oscillations are closer to equilibrium instead of maximum (de)compression at last scattering.
...
we explore the change of the CMB's TT power spectrum with varying one cosmological parameter at the time away from the Planck parameters \eqref{eq_planck2018}:
...
Nevertheless, let us try to understand the ``main effect'' of some parameters:
\begin{itemize}
\item
...
The effect of radiation driving
-- the decay of potentials during compressions in radiation domination so they don't have to fight gravity on the way out and amplifying the peaks --
becomes less pronounced because radiation-matter equality occurs earlier.
Moreover, without dark matter, there would be no wells for the baryons to load,
so the fact that CMB measurements have a second (decompressional) and third (compressional) peak that are of the same height
is clear evidence that dark matter exists.

\item
Increasing today's photon temperature $T_{\gamma 0}$ increases the photon and radiation content of the universe.
This enhances the early-time ISW effect and radiation driving, leading to higher peaks.
...
so power is not amplified at smaller scales, as it was for the photons.
...
only produces an overall multiplication of the entire power spectrum through \eqref{eq_P_primordial}.
...
This is particularly noticeable for the Sachs-Wolfe plateau and the largest-scale modes,
which have been affected very little since early times.

\item
Increasing $Y_p$ increases the number of free electrons $X_e$ (see \cref{fig_free_electron_fraction}),
but decreases the total number density of free electrons $n_e$ (see \eqref{eq_free_electron_fraction_definition}).
Hence, the photon's mean free path and the effect of diffusion damping increases,
...
\item
If we \emph{could} increase $\Omega_{k0}$ (we have only implemented curvature at the background level, but not in the perturbation equations),
it would mainly affect the location of the first peak.
For example, in a closed universe with $\Omega_{k0} < 0 < k$, light rays converge,
...
From the measured CMB spectrum, we know that our Universe is approximately flat.
\end{itemize}

\TODO{understand radiation driving, baryon loading, diffusion damping. mention in perturbation section, so I can refer back here?}

\TODO{Parameter degeneracies: $\Omega_{s0} h^2$, $N_\text{eff}$ and $Y_p$?}

...
\begin{figure}
\centering
\includegraphics[width=0.49\textwidth]{../plots/splinetest_quadratic.pdf}
\includegraphics[width=0.49\textwidth]{../plots/splinetest_cubic.pdf}
\includegraphics[width=0.49\textwidth]{../plots/splinetest_quartic.pdf}
\includegraphics[width=0.49\textwidth]{../plots/splinetest_logarithmic.pdf}
\caption{``Maximum normalized component discrepancies'' (as in \cref{fig_perturb_methods}),
but here comparing perturbation modes $y_i^k(x)$ integrated for precise $k$
to splined perturbations $y_i(x,k)$ from integrating $200$ $k$-values with varying $k$-spacing.
For example, quadratic spacing means that the points $k_i=k_\text{min} + (k_\text{max}-k_\text{min}) \cdot (i/199)^2$ are used in the splining,
while logarithmic spacing means $\log_{10} k_{i}-\log_{10} k_{i-1} = \text{const}$.
For measuring the differences, $100$ $k$-values and $341$ $x$-values have been chosen \emph{at random},
in order to ensure that they do not overlap with values used in the splining.
}
\label{fig_perturb_splines}
\end{figure}

Finally, we need to $(x,k)$-spline the perturbations before computing power spectra in the next milestone.
We have already examined perturbation modes for fixed $k$ in detail, so the remaining question to address is
how the $k$-values in the 2-dimensional spline should be chosen for the spline to be most accurate.
\Cref{fig_perturb_splines} shows that choosing $k$-values with constant logarithmic spacing $\Delta \log_{10} k$ (and greater concentration near small $k$)
yields the lowest interpolation errors.
During rapid late-time oscillations, the discrepancies grow regardless of the $k$-spacing.
\TODO{are late-time discrepancies critical? or do we only need accurate splining up to recombination, or something?}
...
The \textbf{CMB power spectrum} is characterized by the sphere-expansion coefficients
\begin{equation}
...
\label{eq_Cl}
\end{equation}
where $\{\text{AB}\} \in \{\text{TT},\text{EE},\text{TE}\}$ denotes \textbf{t}emperature self-correlation, \textbf{E}-mode polarization self-correlation and their cross-correlation, respectively.
...
\TODO{is $\Theta^T = \Theta$ and $\Theta^E = \Theta^P$?}
\begin{subequations}
\begin{align}
	\Theta^\text{T}_l(x=0,k) &= \phantom{\sqrt{\frac{(l+2)!}{(l-2)!}}} \int_{-\infty}^0 \tilde{S}^\text{T}(x,k) j_l\big(ck(\eta_0-\eta)\big) dx, \\
	\Theta^\text{E}_l(x=0,k) &= \sqrt{\frac{(l+2)!}{(l-2)!}} \int_{-\infty}^0 \tilde{S}^\text{E}(x,k) j_l\big(ck(\eta_0-\eta)\big) dx,
\label{eq_Thetal0}
\end{align}
\end{subequations}
...
The terms in the temperature source function named after the effects they describe or the people who originally studied them:
\begin{itemize}
\item
The \textbf{Sachs-Wolfe effect} describes gravitational redshift of photons that climb out of potential wells
at the last scattering surface (and during reionization) (see \cref{fig_visibility_function}).
Roughly speaking, the visibility function can be approximated as a spike $\tilde{g}(x) \approx \delta(x-x_\text{rec})$ at recombination,
so it contributes
\begin{equation}
	\Theta_l(0,k) \approx \int \delta\big(x-x_\text{rec}\big) \big( \Theta_0 + \Psi + \tfrac14 \Pi \big) \, j_l\big(ck(\eta_0-\eta\big) dx = \big(\Theta_0 + \Psi + \tfrac14 \Pi \big) \, j_l\big(ck(\eta_0-\eta\big) \Big|_{x=x_\text{rec}}.
\end{equation}
Moreover, $j_l(y)$ spikes and becomes nonzero at $y \gtrsim l$, so with $\eta_0 \gg \eta_\text{rec}$ we expect that $\Theta_l(0,k \gtrsim l \, c \eta_0)$.

\item
The \textbf{integrated Sachs-Wolfe (ISW) effect} also describes gravitational redshift of photons,
but of photons traveling through wells that deepen \emph{as} the photons travel through them,
as described by the $(\Psi-\Phi)^\prime$ term.
This leads to additional energy loss.
As we saw in \cref{fig_perturb_all}, the potentials $\Psi$ and $\Phi$ mostly decay during radiation and dark energy domination, and not during matter domination.
Thus, the \textbf{early-time ISW} describes photons in wells that still sink after after recombination, but before matter domination, and shows up on small scales (large $l$ and $k$) that enter the horizon during radiation domination, in time for their potentials to drop.
On the other hand, the \textbf{late-time} ISW describes the same effect for potentials that decay as dark energy takes over, and shows up on large scales (small $l$ and $k$).

\item
The \textbf{Doppler term} contains $(\dot{a} \tilde{g} v_b)^\prime$,
and essentially describes change in the ``photon emitter'' with distance from (last) scattering.
Thus, photons emitted at different times are redshifted by different amounts.

\item
The \textbf{polarization term} contains $\Pi$ and has to do with polarization.
Unlike in Thompson scattering,
...
\begin{itemize}
\item
To plot today's matter power spectrum $P(k)$,
we simply choose a range of $k$ values,
integrate the perturbation modes for all of them,
and calculate \eqref{eq_Pk} at $x=0$.

\item
To plot today's CMB power spectrum $C_l$,
we perform the $k$-integral \eqref{eq_Cl} with the trapezoid method over the grid $1/(c\eta_0) \leq k \leq 4000/(c\eta_0)$ with uniform spacing $\Delta k = 2 \pi / 10 c \eta_0$.
For each $k$-integral, we perform the $x$-integrals \eqref{eq_Thetal0} also with the trapezoid method over the grid $-10 \leq x \leq 0$ with $1000$ uniformly spaced $x$-values \TODO{verify}.

\item
To evaluate the source functions \eqref{eq_S} at arbitrary $x$ and $k$, we make two-dimensional cubic splines of them
after integrating $250$ logarithmically spaced perturbation modes over the integration interval.
This is almost as accurate as integrating every $k$-mode,
as the source function(s) $S(x,k)$ are vary relatively smoothly,
unlike the rapidly oscillating product(s) $S(x,k) j_l(ck(\eta_0-\eta)$.

\item
Calculating the self-correlations $C_l^\text{TT}$ and $C_l^\text{EE}$ involves two nested integrals and takes around a minute,
while the cross-correlation $C_l^\text{TE}$ involves three nested integrals and therefore takes closer to two minutes.
This is with around $175$ $l$-values.
\TODO{how about on a single thread?}

\item
The integration over $x$ and $k$ dominates the runtime of the computation of the CMB power spectrum.
It could probably be sped up, for example if one could subtract the oscillating part of the integrals with known analytical expressions,
or by employing specialized Levin-type quadrature \footnote{See \href{https://github.com/xzackli/OscillatoryIntegralsODE.jl}{https://github.com/xzackli/OscillatoryIntegralsODE.jl}, for example.}.
\end{itemize}
...
\caption{
	...
	compared to data from SDSS, WMAP, ACT and eBOSS (Lyman-$\alpha$ forest?) \TODO{refs}.
...
\Cref{fig_power_spectrum_matter} shows the matter power spectrum computed from integrated perturbation modes.
The agreement with data is rather good.
\begin{itemize}
\item
Modes with $k=k_\text{eq}$ enter the horizon exactly at radiation-matter equality.
This creates a turnover in the power spectrum, diving it into two regimes.

\item
Modes with $k<k_\text{eq}$ enter the horizon later.
As shown in \cref{fig_perturb_all}, the potentials change very little for such modes,
so the power spectrum \eqref{eq_Pk} roughly predicts $P = |\Delta|^2 P_0 \propto \Phi_0^2 k^{n_s} \propto k^{n_s} \approx k^1$,
corresponding to the line with positive slope in the logarithmic plot.

\item
Modes with $k>k_\text{eq}$ enter the horizon sooner.
As shown in \cref{fig_perturb_all}, the potentials for such modes do change significantly from early to late times.
In particular, the potential $\Phi$ decays more for increasing $k$ (smaller-scale modes),
so $P = |\Delta|^2 P_0 \propto \Phi_0^2 k^{n_s}$ instead falls off
in what seems to be a $P \propto k^{-3}$ fall with $\Phi_0 \propto k^{-2}$,
in agreement with the analytical expression in \cite[equation (8.71)]{dodelsonModernCosmology2021}.
\end{itemize}
...
\caption{
	...
	compared to data from Planck \TODO{refs}.
}
\label{fig_power_spectrum_cmb}
\end{figure}

\Cref{fig_power_spectrum_cmb} shows CMB power spectra found by performing the integrals for the coefficients \eqref{eq_Cl}.
The agreement is relatively good, except for the first peaks that are a little lower than Planck's.  \TODO{why?}

Let us take some time to explain the shape of the ``main'' TT power spectrum:
\begin{itemize}
\item
The low-$l$ regime to the left of the first peak is called the Sachs-Wolfe plateau.
At these large angular (and spatial) scales, the modes have entered the horizon only very recently.
They are therefore unaffected by the causal physics that have acted throughout the universe's history,
so this plateau directly reflects the initial conditions and lets us probe inflation.
Assuming a primordial power spectrum that gives equal power to all scales,
the power spectrum therefore appears relatively flat in this regime.

\item
The first peak at $l \approx 10^{2.3}$ reflects the first compression due to baryonic acoustic oscillations.
This peak represents baryons where oscillations due to pressure counteracting gravity
has just reached the maximum of \emph{one} compression \emph{at} the time of recombination, from which the photons stream freely.
Thus, photons must climb out of deep wells, so the temperature fluctuations appear large at this scale.

\item
The first trough at $l \approx 10^{2.6}$ reflects a mode for which the compression/decompression is ``zeroed'' at the time of recombination.
Thus, there are smaller differences in photons streaming towards us at this angular and spatial scales.

\item
The second peak at $l \approx 10^{2.7}$ is a mode that has reached maximum decompression when the photons are released at recombination.
These photons do not have to climb out of potential wells, so they appear hot at these angular/spatial scales.

\item
The next odd and even peaks also represent oscillations at maximum compression and decompression, respectively,
but are lower than the first peaks because baryon acoustic oscillations are damped.
For large $l \gtrsim 1000$, the peaks become successively lower.
This is because on small scales (large $k$ and $l$), the baryon acoustic oscillations are diffusion damped:
photons and baryons are \emph{imperfectly coupled}, so the photons do not follow the baryons perfectly and keep cold and hot regions separate;
instead, they diffuse away from the baryons, hence mixing hot and cold regions, and washing out temperature anisotropies.
The damping is stronger on smaller scales (large $l$) because diffusion washes out anisotropies over short distances before they do so over large distances.
\end{itemize}

Polarization spectrum:
\begin{itemize}
\item
Polarization is associated with presence of quadrupole $\Theta_2$,
must be large enough $k$ to enter horizon before equality.
Have that $\Theta_2$ is in phase with $\Theta_1$, both of which are out of phase with $\Theta_0$.
Since $\Theta_0$ contributes the most to $C_l^\text{TT}$ and $\Theta_2$ contributes the most to $C_l^\text{EE}$,
we see that one has peaks when the other has troughs.
$\Theta_2$
Almost perfectly out of phase with the temperature power spectrum (why?)
\TODO{The main EE power spectrum has peaks that are out of
phase with those in the TT spectrum because the polarization anisotropies are sourced by the
fluid velocity}
...
\begin{itemize}
\item
\TODO{The TE part of the polarization and temperature patterns comes from correlations
between density and velocity perturbations on the last-scattering surface, which can be both positive
and negative, and is of larger amplitude than the EE signal}
\end{itemize}

\begin{figure}
\centering
\includegraphics[width=0.49\textwidth]{../plots/power_spectrum_CMB_varying_h.pdf} \hfill
\includegraphics[width=0.49\textwidth]{../plots/power_spectrum_CMB_varying_Ωb0.pdf} \\
\includegraphics[width=0.49\textwidth]{../plots/power_spectrum_CMB_varying_Ωc0.pdf} \hfill
\includegraphics[width=0.49\textwidth]{../plots/power_spectrum_CMB_varying_Tγ0.pdf} \\
\includegraphics[width=0.49\textwidth]{../plots/power_spectrum_CMB_varying_Neff.pdf} \hfill
...
\includegraphics[width=0.49\textwidth]{../plots/power_spectrum_CMB_varying_Yp.pdf}
...
Finally, \cref{fig_power_spectrum_cmb_varying_parameters} explores the rough dependence of the CMB power spectrum on the different cosmological parameters \eqref{eq_planck2018}.
\begin{itemize}
\item
Increasing $h$ changes several things:
The peaks of the power spectrum shifts to larger scales, which can be explained by photons and sound waves being able to travel longer physical distances in a given time ``for free''.
Intuitively, we can think that more expansion seeks to ``move'' things further apart, hence shifting the spectrum to larger scales.
The power spectrum is also suppressed, as the universe is younger, so anisotropies have had less time to build up.
\TODO{bullshit???}

\item
Increasing the baryon density $\Omega_{b0}$ amplifies the difference between the odd and even acoustic peaks in the power spectrum.
A very obvious way to explain this trend: if there were no baryons, there would be no baryonic acoustic oscillations.
When there are more baryons, the gravitational pull during acoustic oscillations is stronger and makes the compressional odd peaks more pronounced.
The even peaks are suppressed or damped, as it is harder to decompress the fluid ``up from'' the deeper wells (due to increased matter).

\item
Increasing the cold dark matter density $\Omega_{c0}$ has the opposite effect of suppressing the power spectrum particularly the first peaks.
The wells are deeper, so it is tougher for the photon pressure to decompress, and the power spectrum is generally more suppressed.

\item
Increasing today's photon temperature $T_{\gamma 0}$, and thus the photon density $\Omega_{\gamma 0}$, increases temperature fluctuations.
The pressure and decompression generally becomes more violent, leading to less damping and more pronounced temperature fluctuations.

\item
Increasing the effective number of \emph{massless} neutrinos $N_\text{eff}$ increases the radiation content in the universe.
Consequently, the expansion rate \eqref{eq_conformal_hubble_dominated_radiation} at recombination increases.
...
\item
Increasing $Y_p$ decreases the number density of free electrons $n_e$ (although $X_e$ increases, clarify this easily by referring to some equation!),
hence increasing the photons' mean free path and the effect of diffusion damping,
yielding a more suppressed tail.
\end{itemize}

\TODO{understand radiation driving, baryon loading, diffusion damping}

Parameter degeneracies: $\Omega_{s0} h^2$, $N_\text{eff}$ and $Y_p$?

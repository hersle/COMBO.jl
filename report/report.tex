...
\item
\Cref{fig_perturb_methods} shows that integrating only the full system with \texttt{KenCarp4}
...
\Cref{fig_perturb_all} shows four integrated perturbation modes of different scales $0.001/\mathrm{Mpc} \leq k \leq 1/\mathrm{Mpc}$:
\begin{itemize}
\item
...
\item
With $176$ $l$-values and no parallelization,
...
\item
Although we default to $\Delta x = 0.01$ for a ``honest'' calculation for the CMB power spectrum,
note that it is virtually unchanged if we compute it ``dishonestly'' with $\Delta x = 0.05$.
This also cuts the runtime from around $2$ minutes to $0.5$ minutes (see \cref{table_benchmark}).
...
Finally, we summarize the runtime of each module of our program in \cref{table_benchmark}.
Note that we can get a CMB TT power spectrum that fits well with data in only \TODO{find crudest tolerances/resolution of entire program that produces acceptable CMB power spectrum}.

\begin{table}[b]
\centering
\caption{%
	Performance benchmark for each module of our Einstein-Boltzmann solver, as described in the sections of this report.
	The program is run on an expensive laptop from 2022 with no parallelization.
	Resolution and tolerance parameters are found in the implementation details of each section.
}
\label{table_benchmark}
\begin{tabular}{l l l r}
	\toprule
	Section & Module & Variables & Runtime \\
	\midrule
	%Parameter construction & & $2 \, \mathrm{ns}$ \\
	\Cref{sec_background_cosmology} & Background cosmology & & $100 \, \mathrm{\mu s}$ \\
	\Cref{sec_recombination}        & Recombination and reionization & & $2 \, \mathrm{ms}$\\
	\Cref{sec_perturbations}        & Perturbation mode & ($k=0.01/\mathrm{Mpc}$) & $13 \, \mathrm{ms}$ \\
	\Cref{sec_perturbations}        & Perturbation mode & ($k=1   /\mathrm{Mpc}$) & $68 \, \mathrm{ms}$ \\
	\Cref{sec_perturbations}        & Perturbation mode & ($k=100 /\mathrm{Mpc}$) & $67 \, \mathrm{ms}$\\
	\Cref{sec_power_spectra}        & Matter power spectrum & ($200$ $k$s) & $7 \, \mathrm{s}$\\
	\Cref{sec_power_spectra}        & CMB TT power spectrum & ($176$ $l$s, ``honest'' $\Delta x=0.01$) & $110 \, \mathrm{s}$\\
	\Cref{sec_power_spectra}        & CMB TT power spectrum & ($176$ $l$s, ``dishonest'' $\Delta x=0.05$) & $ 30 \, \mathrm{s}$\\
	\bottomrule
\end{tabular}
\end{table}

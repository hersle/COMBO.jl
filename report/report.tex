...
With reionization, $X_e$ ramps back up at late times because increasing radiation from formed stars force the electrons out of the neutral atoms \TODO{motivate in theory, not here}.
...
\TODO{log plot, can talk more then}
...
During recombination, the visibility spikes, as the decrease in free electrons clears the way for the first and largest ``wave'' of photons escaping to today.
This motivates naming the distance they have traveled the ``last scattering surface''.

Reionization gives rise to new free electrons, and thus a more recent, but much smaller last scattering surface.
The electrons are much more dilute now, so the visibility reaches only around $1/50$ of its peak value during decoupling.

As it is calculated directly from the optical depth, the visibility function's numerical implementation also seems sound.

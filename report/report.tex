...
\begin{equation}
	P_\text{primordial}(k) = \frac{2\pi^2}{k^3} \cdot A_s \Big( \frac{k}{k_\text{pivot}} \Big)^{n_s-1}
\label{eq_P_primordial}
\end{equation}

\begin{equation}
	|\Delta|^2 = \cdot P_\text{primordial}(k)
	\quad \text{where} \quad
	\Delta(x,k) = \frac23 \Big( \frac{ck}{H_0} \Big)^2 \frac{\Phi(x,k)}{\Omega_{m0}/a(x)}
\label{eq_P}
\end{equation}
...
\caption{CMB power spectra for varying parameters}

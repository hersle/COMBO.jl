...
\TODO{charge neutral}
\TODO{reorder?}
\TODO{expectation with $k_B T \approx E_{ion}$}
...
\end{align}
\end{subequations}
Unlike the Saha equation, the Peebles equation is a differential equation and needs an initial value.
Their results can be stitched together at a joint $(x, X_e(x))$, which is also used as the initial value for the Peebles equation.
...
\TODO{annotate times with vlines in plots}
...
As expected from the Saha equations \eqref{eq_saha_H_He},
...
Note that without reionization, the Peebles equation causes the free electron density to asymptotically freeze out today at the small values $X_e(0) = \{10^{-3.6}, 10^{-3.7}\}$ with and without Helium.
In contrast, the Saha equation dives straight to $0$ during the final recombination to Hydrogen.

From the plot, we read off the reionization time reported in \cref{table_times2}.
\TODO{define in theory}

\begin{figure}
	\centering
	\includegraphics[scale=0.7]{../plots/optical_depth.pdf}
	\caption{Evolution of the optical depth \eqref{eq_optical_depth} and its derivatives through recombination and reionization history, with or without both Helium and reionization.}
	\label{fig_optical_depth}
\end{figure}

\Cref{fig_optical_depth} shows the corresponding evolution of the optical depth \eqref{eq_optical_depth}.
The optical depth measures the total ``thickness'' of what the most distant light (from the horizon \TODO{?}) reaching us today has traveled through,
so the plot should be interpreted backwards in time (from right to left), and $\tau$ necessarily monotonically increases as we go back in time.
Without reionization, the optical depth steadily increases to the still small value $\tau \approx 10^{-1}$ at $x \approx -7$, just after the last recombination.
With reionization, this value is instead reached very quickly due to reionization, after which it flats out.
...
We obtain the last scattering surface, or time of decoupling, reported in \cref{table_times2}. \TODO{define in theory}

\begin{figure}
	\centering
	\includegraphics[scale=0.7]{../plots/visibility_function.pdf}
	\caption{The visibility function \eqref{eq_visibility_function} and its (scaled) derivatives with or without both Helium and reionization. The main plot best shows its variation during recombination, while the embedded plot magnifies its behavior during reionization. Their numerical integrals are annotated.}
	\label{fig_visibility_function}
\end{figure}

\Cref{fig_visibility_function} shows the visibility function \eqref{eq_visibility_function},
which we interpreted as the probability density of the time that a photon observed today was last scattered.
Indeed, the plot shows that it does indeed integrate to $1$.

There is low visibility before recombination, as photons back then were continuously scattered in the coupled photon-baryon plasma.
As recombination begins, the visibility increases to a maximum, as the decrease in free electrons clears the path for the first ``wave'' of photons hitting us today.
This is the ``last scattering surface'' of most photons that reach us today.
As recombination ends, the visibility function drops to $0$ again, as there are almost no more free electrons to scatter off.

Only with reionization does the visibility increase again at late times, due to the new rise in free electrons.
However, the electrons are much more dilute now, so the visibility is only around $1/50$ of its peak value during recombination.

Finally, we report the sound horizon \eqref{eq_sound_horizon} at decoupling: $s(x_\text{LSS}) = 0.14\,\mathrm{Gpc}$.
...
\begin{tabular}{l c c c c c}
	...
	Event                                                               & $x$     & $a$       & $z$    & $\eta$    & $t$ \\
	\midrule
	Last scattering surface / decoupling ($\tau = 1$)                   & $-6.99$ & $0.0009$  & $1084$ & $0.9\,\mathrm{Gyr}$ & $0.38\,\mathrm{Myr}$ \\

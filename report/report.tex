...
\label{eq_Pk}
\end{equation}
is given by the \TODO{name/gauge?} overdensity
\begin{equation}
...
\label{eq_Cl}
\end{equation}
where $\{\text{AB}\} \in \{\text{TT},\text{EE},\text{TE}\}$ denotes \textbf{T}emperature self-correlation, \textbf{E}-mode polarization self-correlation and cross-correlation, respectively.
We want the power spectrum for $1 \leq l \lesssim 1000$.
When we integrated the perturbations, we cut the system off at $l \lesssim 10$.
The brute force way forward is therefore to significantly increase the size of the system, and necessarily also the computational requirements.
The more elegant way is to compute the present temperature and polarization multipoles from \emph{line of sight integration}:
\TODO{is $\Theta^T = \Theta$ and $\Theta^E = \Theta^P$?}
\begin{subequations}
\begin{align}
	\Theta^T_l(x=0,k) &= \phantom{\sqrt{\frac{(l+2)!}{(l-2)!}}} \int_{-\infty}^0 \tilde{S}^T(x,k) j_l\big(ck\big(\eta_0-\eta\big)\big) dx, \\
	\Theta^E_l(x=0,k) &= \sqrt{\frac{(l+2)!}{(l-2)!}} \int_{-\infty}^0 \tilde{S}^E(x,k) j_l\big(ck\big(\eta_0-\eta\big)\big) dx
\label{eq_Thetal0}
\end{align}
\end{subequations}
with spherical Bessel functions of the first kind $j_l(x)$ and the \textbf{source functions}
\begin{subequations}
\begin{align}
	\tilde{S}^T(x,k) &= \tilde{g} \big( \Theta_0 + \Psi + \tfrac14 \Pi \big) + e^{-\tau} \big( \Psi - \Phi \big)^\prime - \frac{1}{ck} (\aH \tilde{g} v_b)^\prime + \frac{3}{4c^2k^2} \big( \aH (\aH \tilde{g} \Pi)^\prime \big)^\prime, \quad \big( {}^\prime = \tfrac{\partial}{\partial x} \big), \\
	\tilde{S}^E(x,k) &= \phantom{\tilde{g} \big( \Theta_0 + \Psi + \tfrac14 \Pi \big) + e^{-\tau} \big( \Psi - \Phi \big)^\prime - \frac{1}{ck} (\aH \tilde{g} v_b)^\prime +{}} \frac{3}{4 c^2 k^2} \frac{\tilde{g} \Pi}{(\eta_0-\eta)^2}.
\label{eq_S}
\end{align}
\end{subequations}

Finally, note that it is easier to see the $l$-variation of the power spectrum in the conventional quantity
\begin{equation}
	D_l^\text{AB} = \frac{l (l+1)}{2 \pi} \left(\frac{T_{\gamma 0}}{\mu \textrm{K}}\right)^2 C_l^\text{AB}.
\label{eq_Dl}
\end{equation}

...
we can simply integrate every required perturbation $k$-mode and evaluate it at the necessary $x$.
...
so a more efficient way is to instead \emph{spline} $S(x,k)$ on a smaller $k$-grid,
...
One could also try to handle the oscillating integrals with specialized Levin-type quadrature\footnote{See \href{https://github.com/xzackli/OscillatoryIntegralsODE.jl}{https://github.com/xzackli/OscillatoryIntegralsODE.jl}, for example.}.
\end{itemize}
...
\caption{Today's temperature monopoles \eqref{eq_Thetal0} (for different $l$) and its integrand (for $l=10$) evaluated with the trapezoid method on a grid with uniform spacing $\Delta x = 0.01$.}
\label{fig_Theta0}
